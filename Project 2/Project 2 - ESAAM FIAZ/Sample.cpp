...
float   bladeRotationAngle = 0.f;
float   tailRotationAngle = 0.f;
...
Animate( )
{
	...
	bladeRotationAngle = (float)ms / (float) .1;
	tailRotationAngle = ((float)ms / (float) .1) * 3;
...
Display( )
{
	...
	//Set LookAt to either outside the helicopter or from the cockpit

	if (WhichView == INSIDE) {
		gluLookAt(-0.4, 1.8, -4.9,	 -0.4, 1.8, -5.0,	 0., 1., 0.);
	}
	else {
	...
	glRotatef(90., 0., 1., 0.);
	glRotatef(tailRotationAngle, 0., 0., 1.);
	glCallList( TailList );
	glPopMatrix();
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "ESSAM FIAZ - Project 2" );
